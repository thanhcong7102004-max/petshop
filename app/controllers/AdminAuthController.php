...
class AdminAuthController extends Controller
{
    ...
    {
        // Log để debug
        file_put_contents(__DIR__ . '/../../debug_logout.log', date('Y-m-d H:i:s') . " - Logout called\n", FILE_APPEND);
    ...
    {
        // Ngăn chặn output trước khi gửi JSON
        while (ob_get_level()) {
            ob_end_clean();
        }
        ...
        try {
            ...
            if (!$pendingEmail) {
                $this->json(['success' => false, 'message' => 'Phiên đăng ký đã hết hạn. Vui lòng đăng ký lại.']);
                return;
            }
            ...
            if (!$user) {
                $this->json(['success' => false, 'message' => 'Không tìm thấy tài khoản']);
                return;
            }
            ...
            if ($emailSent) {
                ...
                $this->json(['success' => false, 'message' => 'Không thể gửi email. Vui lòng thử lại.']);
            }
        } catch (Exception $e) {
