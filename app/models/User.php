...
class User
{
     ...
     */
    public function updateOTP($userId, $otp, $otpExpiry)
    {
        $sql = "UPDATE users SET otp_code = ?, otp_expiry = ? WHERE id = ?";
        return $this->db->execute($sql, [$otp, $otpExpiry, $userId]);
    }
    ...
    public function resetPassword($token, $newPassword)
    {
        ...
        // Cập nhật mật khẩu, xóa token, kích hoạt tài khoản nếu còn pending
        $updateSql = "UPDATE users SET password = ?, reset_token = NULL, reset_token_expiry = NULL, status = 'active', email_verified = 1 WHERE id = ?";
